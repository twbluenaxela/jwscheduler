...
**Excel 每頁兩週**——直接列印即為每頁兩週的公佈欄版面。每一頁都補滿到相同高度，並以「縮放至 N 頁高」輸出，因此不論列印時實際使用多大的頁面邊界（手機的列印對話框常會自行加大邊界），每頁一律完整兩週，不會出現半週被切到下一頁的情形。電腦版 Excel、手機列印對話框與 Google 試算表皆已實測。
