...
**Excel 每頁兩週**——Excel 主要是可編輯的資料檔。列印時每一頁都補滿到相同高度，並以「縮放至 N 頁高」輸出，因此不需要知道列印時實際的可用頁面高度，也不受邊界設定影響。電腦版 Excel、LibreOffice 與 Google 試算表皆已實測（112 種邊界×分頁組合）；但最終分頁仍由開啟檔案的程式決定，若手機版 Excel 列印結果仍有出入，請改用 PDF 版面。
