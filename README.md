...
| Excel（可編輯表格，列印為每頁兩週） | 全部 / 本月 / 自訂 |
| PDF（每週一頁） | 全部 / 本月 / 自訂 |
| **PDF 版面**（列印用，每頁 1–4 週） | 全部 / 本月 / 自訂 |
| 列印 | 全部 / 本月 / 自訂 |

所有視覺匯出（JPG／PDF／列印／PDF 版面）皆截圖真實的週次卡片，輸出內容與畫面完全一致。

**PDF 版面**——開啟後可用 2×2 的版面選擇器決定每頁要放幾週，或用 ＋／− 增減格數（最多 4 格，預設 2 格上下排列）。左側的 A4 縮圖會即時顯示版面結果，選擇會記在瀏覽器中。

**列印請用 PDF 版面**——PDF 版面是直接拼出 A4 頁面（截圖真實的週次卡片），打開的程式不會重新分頁，所見即所印。

**Excel 每頁兩週**——Excel 主要是可編輯的資料檔。列印時每一頁都補滿到相同高度，並以我們自行計算的縮放比例輸出（不使用「縮放至 N 頁」，因為那會讓 Excel 忽略所有手動分頁線），因此不需要知道列印時實際的可用頁面高度，也不受邊界設定影響。電腦版 Excel、LibreOffice 與 Google 試算表皆已實測（112 種邊界×分頁組合）；但最終分頁仍由開啟檔案的程式決定，若手機版 Excel 列印結果仍有出入，請改用 PDF 版面。
...
在週中編輯模式的導覽列可將週次標記為**一般／特別／大會／暫停**，並填寫說明文字（例如「國際大會」「分區監督探訪」）。輸入說明文字時，系統會依關鍵字自動建議類型；但只要你手動點過類型按鈕，你的選擇一律優先。

| 類型 | 意義 | 呈現方式 |
|---|---|---|
| 一般 | 例行聚會 | 正常卡片 |
| 特別 | 聚會照常，但節目不同（分區監督探訪、特別演講） | 卡片標示琥珀色標籤 |
| **大會**／**暫停** | **當週沒有週中聚會** | 卡片收合為一列提示：「本週聚會暫停 — ○○」 |

大會／暫停週在總覽中顯示為暫停事項列——不計算空缺、不顯示指派槽位。

**在匯出中的處理**：大會／暫停週不會佔用版面格。Excel 中收合為一列提示，PDF 版面中則是一條細提示列，因此其餘週次仍維持你選定的每頁週數（例如選 2 週，就不會因為中間插入一個大會週而錯位）。
