...
**Excel 每頁兩週**——Excel 主要是可編輯的資料檔。每兩個有聚會的週次會放在一個獨立工作表，並設定成 A4 一頁寬 × 一頁高；因此手機版 Excel 套用自己的列印邊界時，只會調整整張工作表的比例，不會再把第二週的最後幾列推到下一頁。月份有 5 週時會產生 3 個工作表；列印整個月份時請在 Excel 的列印範圍選擇「整份活頁簿」。
